...
namespace pylib {
  ...
  public:
    ...
    long AccurateMicrosecond() const { return _tval->tv_sec * 1000000 + _tval->tv_usec; }
    double DecimalSecond() const {
      double ret = _tval->tv_sec * 1000000 + _tval->tv_usec;
      return ret / 1000000;
    }
    // Calculate time
    Timer& Add(const Timer &timer) {
      _tval->tv_sec += timer._tval->tv_sec;
      _tval->tv_usec %= 1000000;
      _tval->tv_usec += timer._tval->tv_usec + _tval->tv_usec / 1000000;
      return *this;
    }
    ...
    Timer operator+(const Timer &timer) {
      Timer newtimer;
      newtimer._tval->tv_usec = this->_tval->tv_usec + timer._tval->tv_usec;
      newtimer._tval->tv_sec = this->_tval->tv_sec + timer._tval->tv_sec + newtimer._tval->tv_usec / 1000000;
      newtimer._tval->tv_usec %= 1000000;
      return newtimer;
    }
    Timer& Sub(const Timer &timer) {
      if ((_tval->tv_sec * 1000000 + _tval->tv_usec) < (timer._tval->tv_sec * 1000000 + timer._tval->tv_usec)) {
        _tval->tv_sec = 0;
        _tval->tv_usec = 0;
      }
      else {
        long timediff = (_tval->tv_sec * 1000000 + _tval->tv_usec) - (timer._tval->tv_sec * 1000000 + timer._tval->tv_usec);
        _tval->tv_sec = timediff / 1000000;
        _tval->tv_usec = timediff % 1000000;
      }
      return *this;
    }
    Timer& operator-=(const Timer &timer) {
    ...
    Timer operator-(const Timer &timer) {
      Timer newtimer;
      if ((this->_tval->tv_sec * 1000000 + newtimer._tval->tv_usec) < (timer._tval->tv_sec * 1000000 + timer._tval->tv_usec)) {
        newtimer._tval->tv_sec = 0;
        newtimer._tval->tv_usec = 0;
      }
      else {
        long timediff = (this->_tval->tv_sec * 1000000 + this->_tval->tv_usec) - (timer._tval->tv_sec * 1000000 + timer._tval->tv_usec);
        newtimer._tval->tv_sec = timediff / 1000000;
        newtimer._tval->tv_usec = timediff % 1000000;
      }
      return newtimer;
    }
  };
